...
def main():
    ...
    # 3. Fit GMM on warmup data
    print(f"\n[3/5] Fitting GMM on warmup data ({WARMUP_DAYS}d before {trade_start.date()})...")
    pool_rows = []
    for t in active:
        warmup_df = all_data[t]
        warmup_df = warmup_df[warmup_df.index < trade_start].dropna(subset=FEATURE_COLS)
        if len(warmup_df) >= 20:
    ...
    for date in sim_dates:
        ...
        for t in active:
            ...
            if p_bull > p_bear and p_bear < 0.35:
                scores_v3[t] = p_bull - p_bear

            # V4 Heuristic
            p_bull, p_bear = heuristic_probabilities(row)
            if p_bull > p_bear and p_bear < 0.35:
                scores_v4[t] = p_bull - p_bear

        w_v3 = capped_temperature_softmax(scores_v3, TEMPERATURE, MAX_STOCK_CAP) if scores_v3 else {}
        w_v4 = capped_temperature_softmax(scores_v4, TEMPERATURE, MAX_STOCK_CAP) if scores_v4 else {}

        target_weights = {t: MIX_V4 * w_v4.get(t, 0.0) + MIX_V3 * w_v3.get(t, 0.0) for t in active}
    ...
    print(f"\n{'='*60}")
    print(f"  V5.0 ENSEMBLE 70-30 RESULTS")
    print(f"{'='*60}")
    for k, v in m.items():
        print(f"  {k:<22}: {v}")
    ...
    ax.plot(dates_plot, eq_arr, color='#2c3e50', linewidth=2.2,
            label=f"V5.0 Ensemble 70-30 (T={TEMPERATURE}, cap={int(MAX_STOCK_CAP*100)}%)")
    ax.axhline(START_CAPITAL, color='#e74c3c', linestyle=':', alpha=0.7)
    ax.fill_between(dates_plot, START_CAPITAL, eq_arr,
                    where=(eq_arr >= START_CAPITAL), alpha=0.08, color='#2c3e50')
    ax.fill_between(dates_plot, START_CAPITAL, eq_arr,
                    where=(eq_arr < START_CAPITAL), alpha=0.12, color='#e74c3c')
    ax.set_title(
        f"V5.0 Ensemble 70-30 — {trade_start.date()} to {trade_end.date()}\n"
    ...
    plt.tight_layout()
    chart_path = os.path.join(CHART_DIR, "backtest_v5_ensemble_equity.png")
    plt.savefig(chart_path, dpi=150)
    plt.close()
    print(f"      Chart -> {chart_path}")

    # Report
    report_path = os.path.join(REPORT_DIR, "backtest_v5_ensemble_report.md")
    lines = [
        "# Version 5.0 Ensemble 70-30 - Backtest Report\n",
        f"> **Trading Period**: {trade_start.date()} to {trade_end.date()}  |  **Universe**: {len(active)} stocks",
        f"> **Blend**: {MIX_V4*100:.0f}% V4 Heuristic + {MIX_V3*100:.0f}% V3 ML GMM",
    ...
    print("\n[DONE] V5.0 Ensemble backtest complete!")
