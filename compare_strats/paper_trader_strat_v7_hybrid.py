...
MID_BASKET = [
    "BEL.NS", "HAL.NS", "CONCOR.NS", "POLYCAB.NS", "MAXHEALTH.NS",
    "PERSISTENT.NS", "OBEROIRLTY.NS", "DIXON.NS", "TATAELXSI.NS", "COFORGE.NS"
]
...
STATE_FILE = "states/portfolio_v7_hybrid.json"
PNL_FILE = "data/pnl_v7_hybrid.csv"
LOG_FILE = "data/trades_v7_hybrid.csv"
CHART_FILE = "charts/pnl_v7_hybrid.png"
REPORT_FILE = "reports/report_v7_hybrid.md"
TRANSITION_COST_PCT = 0.0015  # Blended average friction of 0.15%
...
def main():
    ...
    if state['last_run'] == today:
        print(f"V7 Hybrid strategy already ran today ({today}). Skipping.")
        return
        
    print(f"Running V7 Hybrid daily paper trader for {today}...")
    ...
    if state['regime'] == "SAFE":
        # RUN V5 HYBRID MOMENTUM SATELLITE
        # Rank Large and Mid separately, buy top 3 of each
        mom_large = {}
        ...
        # Capital Allocation: 6 slots of 16.67%
    ...
    if len(pnl_df) >= 2:
        ...
        ax.plot(dates, vals, color='#8b5cf6', linewidth=2.2, label='V7 Apex Hybrid Portfolio (100k)')
        ax.axhline(START_CAPITAL, color='#ef4444', linestyle=':', alpha=0.6)
        ax.set_title(f"Version 7.0 Apex Hybrid Live PnL | as of {today}", fontsize=12, fontweight='bold')
    ...
    report_rows = [
        f"# Live Version 7.0 Apex Hybrid Portfolio Report\n",
