...
# Optimized allocation parameters (same as V3.0 ML for clean comparison)
TEMPERATURE           = 0.15
MAX_STOCK_CAP         = 0.20
BEAR_GUARD            = True

STATE_FILE  = "states/portfolio_v4_heuristic.json"
PNL_FILE    = "data/pnl_v4_heuristic.csv"
LOG_FILE    = "data/trades_v4_heuristic.csv"
CHART_FILE  = "charts/pnl_v4_heuristic.png"
REPORT_FILE = "reports/report_v4_heuristic.md"
...
def main():
    ...
    print(f"\n==================================================")
    print(f"  Live Version 4.0 Heuristic Probabilities | {today}")
    print(f"==================================================")
    ...
    if BEAR_GUARD and not is_nifty_bullish:
        ...
        # Calculate target weights via deterministic Heuristic Probabilities
        scores = {}
        for t, df in stock_dfs.items():
            ...
            p_bull, p_bear = calculate_heuristic_probabilities(row)
    ...
    if len(pnl_df) >= 2:
        ...
        ax.plot(dates, vals, color='#e67e22', linewidth=2.2, label='V4.0 Heuristic Softmax Portfolio (100k)')
        ax.axhline(START_CAPITAL, color='#e74c3c', linestyle=':', alpha=0.6)
        ax.set_title(f"Version 4.0 Heuristic Live PnL | as of {today}", fontsize=12, fontweight='bold')
    ...
    report_rows = []
    report_rows.append(f"# Live Version 4.0 Heuristic Probabilistic Portfolio Report\n")
