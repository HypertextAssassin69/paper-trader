...
STATE_FILE = "states/portfolio_v7_mid.json"
PNL_FILE = "data/pnl_v7_mid.csv"
LOG_FILE = "data/trades_v7_mid.csv"
CHART_FILE = "charts/pnl_v7_mid.png"
REPORT_FILE = "reports/report_v7_mid.md"
TRANSITION_COST_PCT = 0.0020
...
def main():
    ...
    if state['last_run'] == today:
        print(f"V7 Mid strategy already ran today ({today}). Skipping.")
        return
        
    print(f"Running V7 Mid-Cap daily paper trader for {today}...")
    
    # 1. Download Data
    all_tickers = MID_BASKET + [INDEX_TICKER, VIX_TICKER]
    ...
    if state['regime'] == "SAFE":
        # RUN V5 ENSEMBLE SATELLITE (Mid-Caps Momentum)
        ...
        if vix_val < 15.0:
            ...
            leverage = 2.0 - (vix_val - 15.0) * (1.0 / 7.0)  # Scale 2.0x -> 1.0x
        elif vix_val < 25.0:
            leverage = 1.0
        else:
    ...
    if len(pnl_df) >= 2:
        ...
        ax.plot(dates, vals, color='#10b981', linewidth=2.2, label='V7 Apex Mid-Cap Portfolio (100k)')
        ax.axhline(START_CAPITAL, color='#ef4444', linestyle=':', alpha=0.6)
        ax.set_title(f"Version 7.0 Apex Mid-Cap Live PnL | as of {today}", fontsize=12, fontweight='bold')
    ...
    report_rows = [
        f"# Live Version 7.0 Apex Mid-Cap Portfolio Report\n",
