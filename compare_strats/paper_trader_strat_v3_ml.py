...
# V3.0 ML Fix-B Parameters
TEMPERATURE           = 0.30  # Raised from 0.05 -> moderate concentration
MAX_STOCK_CAP         = 0.15  # Hard ceiling of 15% per stock
BEAR_GUARD            = True  # Nifty EMA-50 circuit breaker

STATE_FILE  = "states/portfolio_v3_ml.json"
PNL_FILE    = "data/pnl_v3_ml.csv"
LOG_FILE    = "data/trades_v3_ml.csv"
CHART_FILE  = "charts/pnl_v3_ml.png"
REPORT_FILE = "reports/report_v3_ml.md"
...
def main():
    ...
    print(f"\n==================================================")
    print(f"  Live Version 3.0 ML Probabilistic Trader | {today}")
    print(f"==================================================")
    ...
    for t in TICKERS:
        ...
        # Collect historical feature vectors (excluding the last 5 days to avoid target leakage)
    ...
    # Identify which GMM components are Bearish, Choppy, and Bullish
    preds = gmm.predict(scaled_pooled)
    component_returns = []
    for c in range(3):
        avg_ret = all_pooled_features[preds == c, 0].mean() # column 0 is Return_Mean_Slow
        component_returns.append((c, avg_ret))
    component_returns.sort(key=lambda x: x[1])
    
    bearish_idx = component_returns[0][0]
    choppy_idx = component_returns[1][0]
    bullish_idx = component_returns[2][0]
    print(f"  GMM Mapped States: Bearish={bearish_idx}, Choppy={choppy_idx}, Bullish={bullish_idx}")
    ...
    if BEAR_GUARD and not is_nifty_bullish:
        ...
        # Calculate target weights via GMM Probabilities
        scores = {}
        for t, df in stock_dfs.items():
            row = df.iloc[-1]
            price = row['Close']
            
            # Check if the stock has valid features today
            ...
            # Eligibility: Bull probability > Bear probability and Bear probability < 35%
            if p_bull > p_bear and p_bear < 0.35:
                scores[t] = p_bull - p_bear

        # Capped Temperature Softmax Allocations
        target_weights = {t: 0.0 for t in TICKERS}
        if scores:
            target_weights_dict = capped_temperature_softmax(scores)
            ...
            if t in current_prices:
                ...
                if target_shares < current_shares:
                    ...
                    trade_rows.append({
                        ...
                        'value': round(val_to_sell, 2), 'reason': f'ML Rebalance to target weight={target_weights.get(t,0.0):.3f}'
                    })
                ...
                if target_shares > current_shares:
                    ...
                    if shares_to_buy > 0:
                        ...
                        trade_rows.append({
                            ...
                            'value': round(shares_to_buy * price, 2), 'reason': f'ML Rebalance to target weight={target_weights.get(t,0.0):.3f}'
                        })
    ...
    if len(pnl_df) >= 2:
        ...
        ax.plot(dates, vals, color='#9b59b6', linewidth=2.2, label='V3.0 ML Softmax Portfolio (100k)')
        ax.axhline(START_CAPITAL, color='#e74c3c', linestyle=':', alpha=0.6)
        ax.set_title(f"Version 3.0 ML Live PnL | as of {today}", fontsize=12, fontweight='bold')
    ...
    report_rows = []
    report_rows.append(f"# Live Version 3.0 ML Probabilistic Portfolio Report\n")
